...
$f_{Y||X,S}(y||x,s) = exp\[y\xi(x,s)- \bf{B}{\xi(x,s)} + \bf{C}(y)\]$, where $\xi$ is natural parameter.
...
$$c(\sum_{k=1}^{p}\sum_{j \in J}\theta^{2}_{kj}  + \sum_{m \in \mathcal{M}}\gamma_m^{2}) \leq || \sum_{k=1}^{p}\sum_{j \in J} \theta_{kj}U_{kj}$$ 
$$ + \sum_{m \in \mathcal{M}}\gamma_m B_{m}^{\ast}||^{2} \leq C(\sum_{k=1}^{p}\sum_{j \in J}\theta^{2}_{kj} + \sum_{m \in \mathcal{M}}\gamma_m^{2})$$
