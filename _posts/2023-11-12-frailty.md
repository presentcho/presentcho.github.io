...
Note that $\(S(X_i) = \exp\{ -R_{ij}(x|u_i) \}\)$ 

and $\(S(D_i) = \exp\{ -\Lambda_{ij}(y|u_i) \}\)$, 

where $\(R_{ij}(x|u_i) = u_iR_0(x) \exp( \beta_1Z_{ij})\)$ 

and $\(\Lambda_{ij}(x|u_i) = u_i^{\alpha}\Lambda_0(x)\exp(\beta_2Z_{ij})\).$
